...
> ${bilgilendirme[Math.floor(Math.random()*bilgilendirme.length)]}`)], components: [buttonSatir]}).then(async (x) => {
                ...
                collector.on('collect', async i => {
                  ...
                  if(i.values[0] === 'webpanel') {
                    if(i.user.id != "1187120543950979102") return await i.reply({content: `${cevaplar.prefix} Sunucu sahibi veya da bot sahibi site üzerinden erişim sağlayabilir..`, ephemeral: true}),
                    message.react(message.guild.emojiGöster(emojiler.Onay) ? message.guild.emojiGöster(emojiler.Onay).id : undefined),
                    x.delete().catch(err => {})
                    await i.reply({content: `Sizi yönlendiriyorum...`, ephemeral: true})
                  }
                  if(i.values[0] === 'sure') {
                    if(i.user.id != "1187120543950979102") return await i.reply({content: `${cevaplar.prefix} Sistemde profilin girili olmadığı için süre ve ödeme işlemlerine ulaşamazsın.`, ephemeral: true}),
                    message.react(message.guild.emojiGöster(emojiler.Onay) ? message.guild.emojiGöster(emojiler.Onay).id : undefined),
                    x.delete().catch(err => {})

                    await i.reply({embeds:[new genEmbed().setColor("DARK_AQUA").setDescription(`${message.guild.emojiGöster(emojiler.Icon)} **Merhaba!** Sayın. Takachi (\`rewenxrd\`), ${message.guild.name} (\`ÖMÜR BOYU - LİFE END\`) Sunucusunun ödeme, süre ve paket işlemleri için aşağıda bilgilendirmesi yapılmış ufak kategorilerden kontrol edebilir veya düğmelerden işlem yaparak yada websitesi üzerinden güncelleme gibi işlemleri yapabilirsiniz.`)
                    .addField(`\` ••❯ \` Ödeme Durumu`, `\` • \` **Ödeme Tipi**: Sanal Pos (\`VISA\`)\n\` • \` **Ödeme Tarihi**: 16.12.2021 17:42\n\` • \` **Onay Tarihi**: 16.12.2021 17:43`, true)
                    .addField(`\` ••❯ \` Lisans Bilgisi`, `\` • \` Lisans Tipi: \`Business\`\n\` • \` Lisans Numarası: \`weise (Owner)\``, true)], components: [ödeme],ephemeral: true})
