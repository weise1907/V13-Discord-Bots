...
module.exports = {
  ...
  onLoad: function (client) {
    ...
    client.weiseSaatYap = (date) => { return moment.duration(date).format('H'); };
  },
  ...
  onRequest: async function (client, message, args) {
    let barSystem = [
      ...
      {id: "diger", başlamaBar: "1203458645459861524", doluBar: "doluBar", doluBitişBar: "doluBitisBar", boşBitişBar:emojiler.Terfi.boşBitişBar, boşBar:emojiler.Terfi.boşBar, başlangıçBar: emojiler.Terfi.başlangıçBar},]
    ...
    if(görevBul) {
      ...
      if(data.taskVoiceStats) {
        data.taskVoiceStats.forEach((value, key) => {
            genelses += value
            if(key == kanallar.publicKategorisi) public += Number(client.weiseSaatYap(value))
            if(key == kanallar.streamerKategorisi) public += Number(client.weiseSaatYap(value))
            if(key == kanallar.registerKategorisi) register += Number(client.weiseSaatYap(value))
        });
      };
        ...
        if(button.customId === "gorev_bilgisi") {
          ...
          if(görevBul.publicVoice >= 1) {
           ...
           if(görevBul.AllVoice >= 1) {
             embedcik.addFields({ name: `${görevBul.AllVoice} Saat Tüm Ses Kanallarında Takıl!`, value: `${Number(client.weiseSaatYap(genelses)) >= görevBul.AllVoice ? message.guild.emojiGöster(emojiler.Görev.OK) : message.guild.emojiGöster(emojiler.Görev.Mesaj) } ${yönetimBar(Number(client.weiseSaatYap(genelses)) ? Number(client.weiseSaatYap(genelses)) : 0, görevBul.AllVoice, 5, Number(client.weiseSaatYap(genelses)))} \`${Number(client.weiseSaatYap(genelses)) >= görevBul.AllVoice ? `Tamamlandı!`: `${Number(client.weiseSaatYap(genelses))} saat / ${görevBul.AllVoice} saat`}\``})
           }
