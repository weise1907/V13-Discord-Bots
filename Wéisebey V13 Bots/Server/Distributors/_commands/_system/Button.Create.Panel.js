...
${uye} isimli üyenin <t:${String(Date.now()).slice(0,10)}:F> tarihinde aşağıda istek veya önerisi belirtilmiştir.`)
      ...
      if(modal.customId == "botsorun") {
        ...
        logKanalı.send({content: `<@1187120543950979102>`, embeds: [embed.setDescription(`**Merhaba!** ${ayarlar.serverName} Yönetimi
${uye} isimli üyenin <t:${String(Date.now()).slice(0,10)}:F> tarihinde aşağıda bot sorunu bildirdi.`)
        .addField(`İçerik`, `${text}`)
        ]})
        await modal.deferReply({ ephemeral: true })
        await modal.followUp({content: `Başarıyla bot sorunu weise'a iletilmiştir. Teşekkür Ederiz! ${guild.emojiGöster(emojiler.Onay)}` , ephemeral: true })
      }
      ...
      if(i.customId == "sorunÇözmeci") {
       ...
       if(i.customId == "basvurucuk") {
          ...
          .addComponents(
            ...
            .setMinLength(5)
            .setMaxLength(100)
            .setPlaceholder('Örn: weise#0001/ID')
            .setRequired(false),
            new TextInputComponent()
...
\` 2. Adım \` Kayıt yapmak için \`.kayıt <@weise/ID> <İsim> <Yaş>\` komutu ile kayıt işlemine başlarsın orda konuştuğun üyenin cinsiyetini belirleyerek kayıt işlemini gerçekleştirirsin ve **5 Saniye** sonra Sohbet odalarına otomatik olarak üyeyi taşıma işlemi yapar.
...
await message.react(message.guild.emojiGöster(emojiler.Onay) ? message.guild.emojiGöster(emojiler.Onay).id : undefined).catch(err => {})
                  ...
                  if(i.customId == "yöneticiPaneli") {
                    ...
                    let Row = new MessageActionRow().addComponents(
                      new MessageSelectMenu()
                      .setCustomId("weiseyöneticipaneli")
