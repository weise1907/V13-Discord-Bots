...
module.exports = {
    Isim: "güvenlik",
    Komut: ["yarram", "guard-settings","guards","weiseguard","guard"],
    Kullanim: "",
    Aciklama: "",
    Kategori: "-",
    Extend: true,
...
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
    .setFooter(`Bir üyeyi beyaz listeye ekleyip/çıkartmak için ${global.sistem.botSettings.Prefixs[0]}guard <@weise/ID> komutunu kullanabilirsiniz.`)
    if(!uye) {
        message.channel.send({embeds: [anaMenu], components: [oneRows]}).then(async (msg) => {
            ...
            collector.on('collect', async (i) => {
              ...
              if(i.customId == "guard_manager") {
                ...
                let weiseRow = new MessageActionRow().addComponents([
                    new MessageButton()
                ...
                msg.delete().catch(err => {})
                message.channel.send({embeds: [anaMenu], components: [weiseRow, backMenu]}).then(async (gmsg) => {
                    var filter = (i) => i.user.id == message.member.id
                    ...
                    collector.on('collect', async (i) => {
                        ...
                        if(i.customId == "guardState") {
                            ...
                            if(guardSettings.guildProtection) {
                                weiseRow.components[2].setDisabled(true)
                                weiseRow.components[3].setDisabled(true)
                                weiseRow.components[1].setDisabled(true)
                                weiseRow.components[4].setDisabled(true)
                                weiseRow.components[0].setStyle("DANGER").setLabel(`Güvenliği Aç`)
                                ...
                                weiseRow.components[1].setDisabled(false)
                                if(guardSettings.limit) weiseRow.components[2].setDisabled(false)
                                weiseRow.components[3].setDisabled(false)
                               if(ayarlar.SMS) weiseRow.components[4].setDisabled(false)
                                weiseRow.components[0].setStyle("SECONDARY").setLabel(`Güvenliği Kapat`)
                        ...
                        if(i.customId == "limitState") {
                            ...
                            if(guardSettings.limit) {
                                weiseRow.components[2].setDisabled(true)
                                weiseRow.components[1].setStyle("DANGER").setLabel(`Limiti Aç`)
                                await GUARD_SETTINGS.updateOne({guildID: sistem.SERVER.ID}, { $set: {"limit": false }}, {upsert: true})
                            } else if(!guardSettings.limit) {
                                weiseRow.components[2].setDisabled(false)
                                weiseRow.components[1].setStyle("SECONDARY").setLabel(`Limiti Kapat`)
...
Olarak ayarlandı.`)], components: [row]}).then(async (weise) => {
    ...
    collector.on('end', i => weise.delete().catch(err => {}))
    collector.on('collect', async (i) => {
        weise.delete().catch(err => {})
        let kom = client.commands.find(x => x.Isim == "güvenlik")
        kom.onRequest(client, message, args)
        i.deferUpdate().catch(err => {})
                        ...
                        if(i.customId == "smsState") {
                            ...
                            if(guildSettings) ayarlar = guildSettings.Ayarlar
                            if(ayarlar.SMS) {
                                weiseRow.components[3].setStyle("DANGER").setLabel(`SMS/MMS Aç`)
                                weiseRow.components[4].setDisabled(true).setStyle("SUCCESS").setLabel(`SMS/MMS Gerekli`)
                                ...
                                weiseRow.components[4].setDisabled(false).setLabel(`WhatsApp/Telegram Aç`).setStyle("DANGER")
                                weiseRow.components[3].setStyle("SECONDARY").setLabel(`SMS/MMS Kapat`)
                        ...
                        if(i.customId == "wpState") {
                            ...
                            if(ayarlar.WHATSAPP | ayarlar.TELEGRAM) {
                                weiseRow.components[4].setStyle("DANGER").setLabel(`WhatsApp/Telegram Aç`)
                                await GUILD_SETTINGS.updateOne({guildID: sistem.SERVER.ID}, { $set: {"Ayarlar.WHATSAPP": false, "Ayarlar.TELEGRAM": false }}, {upsert: true})
                                
                            } else if(!ayarlar.WHATSAPP || !ayarlar.TELEGRAM) {
                                weiseRow.components[4].setStyle("SECONDARY").setLabel(`WhatsApp/Telegram Kapat`)
...
**Son Güvenlik Kayıtları**
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
                        gmsg.edit({embeds: [anaMenu], components: [weiseRow, backMenu]}).catch(err => {})  
                        i.deferUpdate().catch(err => {})
                    })
                })
              }
