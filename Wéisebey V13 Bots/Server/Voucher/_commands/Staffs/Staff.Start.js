...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let yetkiliroller = ["id1","id2","id3"] //Yetkili Rolleri


    if(!yetkiliroller.some(x => message.member.roles.cache.get(x)) && !message.member.permissions.has('ADMINISTRATOR')) return message.reply(cevaplar.noyt).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
    ...
    if(message.member.permissions.has('ADMINISTRATOR') || roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku)) || roller.üstYönetimRolleri.some(oku => message.member.roles.cache.has(oku))) {
      ...
      return uye.roles.add(roller.başlangıçYetki).then(x => {
        ...
        uye.roles.add("verilecekilkrol")//3. rol
    ...
    await message.channel.send({content: uye.toString(), embeds: [embed], components: [Row]}).then(async (msg) => {
      ...
      collector.on('collect', async (i) => {
        if(i.customId == "OK") {
          ...
          if(yetkiliLog) yetkiliLog.send({embeds: [embed.setDescription(`${uye} isimli üye <t:${String(Date.now()).slice(0, 10)}:R> ${message.author} tarafından yetkili olarak başlatıldı.`)]})      
          client.Upstaffs.addPoint(message.member.id,_statSystem.points.staff, "Yetkili")
          let verilcekRol = [roller.başlangıçYetki, roller.altilkyetki, "verilecekrol"]//3.rol
          uye.roles.add(verilcekRol).then(x => client.Upstaffs.addPoint(uye.id,"1", "Bonus"))
          message.member.Leaders("yetki", _statSystem.points.staff, {type: "STAFF", user: uye.id})
          i.deferUpdate().catch(err => {})
          let yetkilinindatası = await Users.findOne({
            _id: uye.id
          });
          let adamınismi = yetkilinindatası ? yetkilinindatası.Name : ""
          //uye.setNickname(adamınismi)
        }
        if(i.customId == "NO") {
